...
//Inside the node server, the env file is to contain the database URL.
...
//Here we are creating a route for verifying email
//The main idea is that when user click on this link, we will automatically reset user.token to null and change user.verified to become true
//Only if the user.verified is true, we can enter the neighbor main system. 
app.use('/api/verify-email',async (req,res)=>{
  try{
      ...
      if(user){
          ...
          console.log("Email has been verified before")
      }
  }
  catch(err){
      console.log(err)
